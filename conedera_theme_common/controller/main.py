...
class ProductBrandController(http.Controller):

    @http.route('/snippet/products_by_brand', type='json', auth='public', website=True)
    def products_by_brand(self, brand_id=None, limit=16):
        # Validación de marca
        ...
        for product in products:
            result.append({
                ...
                'image_1920': product.image_1920 and f"/web/image/product.template/{product.id}/image_1920" or "/web/static/img/placeholder.png",
                'brand': product.dr_brand_value_id.name if product.dr_brand_value_id else "",
            })
        return result

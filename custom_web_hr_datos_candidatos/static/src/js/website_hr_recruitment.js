...
publicWidget.registry.MultistepForm = publicWidget.Widget.extend({
    ...
    async _getFamilyBlock(parentesco, index) {
        ...
        const noTieneBlock = ["Padre", "Madre", "Conyugue"].includes(parentesco) ? ` 
            <div class="form-check"> <input class="form-check-input fam-no-tiene" type="checkbox" name="famNoTiene_${index}" value="1" /> 
                <label class="form-check-label fs-6"> No tiene </label> 
            </div> ` : "";

        const estadoFamiliarBlock = `
    ...
    _onNextClick(ev) {
        ...
        if (this._validateCurrentStep1()) {
            ...
            this.$('#hr-hijos').prop('readonly', true);
            if (!isNaN(numHijos)) {
                if (this.$('input[name="numHijos_hidden"]').length === 0) {
                    this.$('#hr-hijos').after(
                        `<input type="hidden" name="numHijos" value="${numHijos}"/>`
                    );
                } else {
                    this.$('input[name="numHijos_hidden"]').val(numHijos);
                }
            }

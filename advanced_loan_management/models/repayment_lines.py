...
class RepaymentLine(models.Model):
    ...
    name = fields.Char(string="Plan", default="/", readonly=True,
                       help="Número de reembolso: del plan")
    ...
    date = fields.Date(string="Fecha de pago", required=True,
                       ...
                       readonly=True,
                       help="Fecha de pago")
    amount = fields.Float(string="Cantidad USD", required=True, help="Cantidad",
                          digits=(16, 2))
    interest_amount = fields.Float(string="Interés", required=True,
                                   help="Cantidad de interés", digits=(16, 2))
    total_amount = fields.Float(string="Total_Plan", required=True,
                                help="Cantidad total del Plan", digits=(16, 2))
    loan_id = fields.Many2one('loan.request', string="Loan Ref.",
                              help="Loan",
                              readonly=True)
    state = fields.Selection(string="Estado",
                             selection=[('unpaid', 'Pendiente'),
                                        ('invoiced', 'Facturado'),
                                        ('defeated', 'Vencida'),
                                        ('paid', 'Pagado')],
        ...
        for rec in self:
            ...
            for line in loan_lines_ids:
                if line.date < rec.date and line.state in \
                        ('unpaid', 'invoiced','defeated'):
                    message_id = self.env['message.popup'].create(
                        {'message': (
                            "Tienes importes pendientes")})
                    return {
                        'name': 'Pagos',

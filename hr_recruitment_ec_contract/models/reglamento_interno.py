...
class HrEcReglamentoInterno(models.Model):
    ...
    _description = "Acta de recepción del reglamento interno"
    ...
    company_config_id = fields.Many2one("empresa.empresa", related="employee_id.company_config_id", string="Empresa Afiliada", store=True, readonly=True)
    request_date = fields.Date(string="Fecha de recepción", required=True, default=fields.Date.today)
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get("name", _("Nuevo")) == _("Nuevo"):
                vals["name"] = sequence.next_by_code("hr.ec.reglamento.interno") or _("Nuevo")
        return super().create(vals_list)

    def action_print_request(self):
        self.ensure_one()
        return self.env.ref("hr_recruitment_ec_contract.action_report_reglamento_interno").report_action(self)

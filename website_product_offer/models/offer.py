...
class WebsiteSaleOffer(models.Model):
    ...
    def _convert_to_quotation(self, price_field):
        ...
        # Validar que todos los productos tengan
        # un precio válido antes de crear el presupuesto.
        for line in self.line_ids:
    ...
    def action_send_counter(self):
        ...
        if invalid_lines:
            ...
            raise UserError(
                _(
                    "Ingrese un precio de contraoferta "
                    "mayor que cero para los siguientes "
                    "productos: %s",
                    products,
                )
            )
        ...
        self.message_post(
            body=_(
                "Se envió una contraoferta para "
                "%s productos.",
                len(self.line_ids),
            ),
            subtype_xmlid="mail.mt_note",
    ...
    def action_customer_accept_counter(self):
        ...
        if invalid_lines:
            ...
            raise UserError(
                _(
                    "Las siguientes líneas no tienen "
                    "un precio de contraoferta válido: %s",
                    products,
                )
            )
